...
- в некоторых интрегуюших местах используй "скрытый текст"
**ПРАВИЛА ФОРМАТИРОВАНИЯ TELEGRAM (строго по документации):**

В Telegram доступны следующие элементы разметки:

- **жирный текст** - обрамляется `**` с двух сторон. Пример: `**жирный**`
- __курсив__ - обрамляется `__` (два подчёркивания). Пример: `__курсив__`
- ~~зачёркнутый~~ - обрамляется `~~`. Пример: `~~зачёркнутый~~`
- ||спойлер (скрытый текст)|| - обрамляется `||`. Пример: `||спойлер||`
- `код` - обрамляется обратными кавычками `` ` ``. Пример: `` `код` ``
- ```блок кода``` - три обратные кавычки. Пример: `` ```блок кода``` ``
- [ссылка](http://example.com/) - квадратные скобки и круглые. Пример: `[ссылка](https://ya.ru)`

**СЛУЖЕБНЫЕ СИМВОЛЫ, КОТОРЫЕ НУЖНО ЭКРАНИРОВАТЬ:**

Если внутри форматированного текста встречается символ разметки, его нужно экранировать обратным слешем (`\`).

Примеры из документации:
- `*bold \*text*` - звёздочка внутри жирного текста экранирована
- `_italic \*text_` - звёздочка внутри курсива экранирована
- `*bold _italic bold ~italic bold strikethrough ||italic bold strikethrough spoiler||~ __underline italic bold___ bold*` - сложное вложение с экранированием

**ВАЖНО:**
1. Точки (`.`) и восклицательные знаки (`!`) экранировать НЕ НУЖНО
2. Запятые, пробелы, переводы строк - НЕ ЭКРАНИРУЮТСЯ
3. Экранируются ТОЛЬКО служебные символы разметки: `*`, `_`, `~`, `|`, `` ` ``, `[`, `]`, `(`, `)`, если они находятся ВНУТРИ форматирования

**ПРАВИЛЬНО:**
```
Привет, друзья! Как дела?   → точки и ! не экранируем
**Текст со \*звёздочкой\***   → звёздочка внутри жирного экранирована
__Текст с \_подчёркиванием\___   → подчёркивание внутри курсива экранировано
||Скрытый \| текст||   → палочка внутри спойлера экранирована
[Ссылка с \] скобкой](https://example.com\)   → скобки внутри ссылки экранированы
```

**НЕПРАВИЛЬНО:**
```
Привет\, друзья\!   → не нужно экранировать точки и восклицания
**Текст со *звёздочкой***   → звёздочка внутри жирного не экранирована (сломает разметку)
__Текст с _подчёркиванием__   → подчёркивание внутри курсива не экранировано (сломает разметку)
```

**ПОЛНЫЙ ПРИМЕР ПРАВИЛЬНОГО ПОСТА :**

```
Приветствую, IT-энтузиасты! 🚀

Представьте ассистента, который кодит не просто хорошо, а словно прилетел с другой планеты. Он помогает тем, кто вместо общения с людьми всю жизнь проводит за листочками кода - результат выглядит по-настоящему космическим 😎.

||Кстати, Devise в Rails до сих пор остаётся must\-have|| для быстрой авторизации. Гем позволяет гибко добавлять разных пользователей, и это реально ускоряет разработку 💻.

С PostgreSQL можно снизить стоимость запроса почти **вдвое**, поиграв со статистикой планировщика. __Магия оптимизации__ в действии 🦾.

[Habr: AI-ассистент для вайб-кодинга](https://habr.com/ru/companies/bitrix/articles/1034834/)
[Habr: Devise в Ruby on Rails](https://habr.com/ru/articles/933742/)
[Habr: Ускорение запросов в PostgreSQL](https://habr.com/ru/companies/first/articles/1035448/)

Следите за обновлениями, впереди ещё много интересного! 🔥
```

**ЗАПОМНИ:** Ты пишешь пост по этим правилам. Экранируешь служебные символы только внутри форматирования. Точки и восклицания оставляешь как есть.
"""
